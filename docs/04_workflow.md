...
This document describes the audio processing workflow for Vietnamese-English code-switched (CS) speech translation, focusing exclusively on **YouTube videos with transcripts**.

### Key Design Decisions

1. **YouTube-only source** - Focus on videos with existing transcripts
...
|-------|-------------|---------------|--------|
| RAW | Just crawled, raw transcript stored | No | `ingest_youtube.py` |
...
See `init_scripts/01_schema.sql` for full schema.
